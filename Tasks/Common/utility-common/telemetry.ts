 ...
 */
export function emitTelemetry(feature: string, taskSpecificTelemetry: any) {
    try {
        ...
        if (semver.gte(agentVersion, '2.120.0')) {
            ...
            console.log("##vso[telemetry.publish area=Packaging;feature=%s]%s",
                feature,
                JSON.stringify(copy));
        } else {
 ...
 */
export function logStderr(feature:string, exitCode: number, stderr: string) {
    try {
        ...
        emitTelemetry(feature, nugetExecResults);
    } catch (err) {
        tl.debug(`Unable to log telemetry. Err:( ${err} )`);
    }
}